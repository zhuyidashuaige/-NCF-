...
(function(){
  ...
  // 新：在开幕式隐藏完成后，再“武装”一次性点击监听来弹出邀请弹窗
  function armInviteAfterOpeningHidden(){
    // 使用 setTimeout(0) 避免与触发隐藏的同一次点击冲突
    setTimeout(()=>{
      const onFirstInteract = ()=>{
        if(!gameInviteModal.classList.contains('show')){
          gameInviteModal.classList.add('show');
        }
      };
      // once:true 确保只触发一次
  ...
  function endOpening(){
    if(!opening.classList.contains('hide')){
      ...
      // 等开幕式过渡完毕后再武装点击触发（避免用同一次点击弹出）
      const onTransitionEnd = ()=>{
        opening.removeEventListener('transitionend', onTransitionEnd);
        armInviteAfterOpeningHidden();
      };
      opening.addEventListener('transitionend', onTransitionEnd, { once:true });
      // 兜底：某些浏览器可能不触发 transitionend，这里 1.3s 后武装
      setTimeout(()=>{
        try{ opening.removeEventListener('transitionend', onTransitionEnd); }catch(e){}
        armInviteAfterOpeningHidden();
      }, 1300);
    }
  }
